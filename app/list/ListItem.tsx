...
export default function ListItem({ result, user }: { result: ItemType[]; user: Session | null }) {
  // console.log('listItem sesssssssion user', user?.user);
  return (
    <>
      {result.map((item, idx) => (
        <div
          ...
          */}
          ...
          />
          <p>{item.author}</p>
          {user && user.user?.email === item.author ? (
            <button
              onClick={(e) => {
                ...
                //     console.error(error);
                //   });
                fetch(`/api/post/delete?_id=${item._id.toString()}`, { method: 'DELETE' }).then(() => {
                  //@ts-ignore
                  e.target.parentElement.style.opacity = 0;
                  setTimeout(() => {
                    //@ts-ignore
